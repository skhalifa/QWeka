    ...
    logger.info("Shadi: getDrillFunctionFromOptiqCall");
      ...
      if (functionName.equals("extract")) {
        ...
        return first;
          ...
          qdmArgs.add(args.get(0));
          //Rest of args are to be concatenated and passed as a csv string
          if(args.size() == 2){
          qdmArgs.add(args.get(1));
          } else if(args.size()>2) {
          List<LogicalExpression> concatArgs = Lists.newArrayList();
          concatArgs.add(args.get(1));
          LogicalExpression comma = new  ValueExpressions.QuotedString(",", ExpressionPosition.UNKNOWN);
          concatArgs.add(comma);
          LogicalExpression first = FunctionCallFactory.createExpression("concat", concatArgs);
          concatArgs = Lists.newArrayList();
          concatArgs.add(first);
          concatArgs.add(args.get(2));
          first = FunctionCallFactory.createExpression("concat", concatArgs);
